...
public class PokerHands {
    ...
    private void testOutput(int[] poker){
        for(int i : poker){
            System.out.print(i + "\t");
        }
    ...
    private int patternCheck(int[] poker, String[] patterns){
        ...
        // 7. 투 페어 : 두 쌍의 카드가 같은 값을 가지는 경우 ... ( 동일한 경우 나머지 카드 값으로 )
            ...
            // poker[0] == poker[1] && poker[2] == poker[3] ? poker[4] : poker[0]
            winNo = 7;
        }
...
Map<String, Integer> pokers = new HashMap<String, Integer>();

pokers - key: Card No. / value: Count;
